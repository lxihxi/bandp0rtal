...
const TYPE_STYLE: Record<string, string> = {
  show: 'bg-red-950 text-red-300', probe: 'bg-blue-950 text-blue-300',
  meeting: 'bg-yellow-950 text-yellow-300', deadline: 'bg-orange-950 text-orange-300', other: 'bg-gray-800 text-gray-400',
}
...
export default function CalendarPage() {
  ...
  const deleteEvent = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['events'] }); qc.invalidateQueries({ queryKey: ['next-events'] }); qc.invalidateQueries({ queryKey: ['dashboard-stats'] }) },
  })

  function handleDayClick(dateStr: string) {
    setPrefillDate(dateStr + 'T20:00')
    setShowForm(true)
  }
  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <>
        <DetailPanel title={fresh.title} onBack={() => setView(view === 'detail' ? 'month' : 'list')} onEdit={() => setView('edit')}>
          <Card>
            <CardBody className="space-y-4">
              <div className="flex flex-wrap gap-2 items-center">
                <span className={`text-xs px-2 py-1 rounded font-medium ${TYPE_STYLE[fresh.type]}`}>{TYPE_LABEL[fresh.type]}</span>
                <span className="text-sm text-gray-300">{formatDate(fresh.date)}</span>
                <span className="text-sm text-gray-500">{formatTime(fresh.date)} Uhr</span>
              </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setPrefillDate(''); setShowForm(true) }} className="flex items-center gap-1.5 px-3 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded transition-colors">
                    ...
                    <div key={event.id} className="flex items-center gap-4 px-4 py-3 hover:bg-white/5 cursor-pointer" onClick={() => { setSelected(event); setView('detail') }}>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                        ...
                        <RowActions actions={[
                          ...
                          { label: 'Bearbeiten', onClick: () => { setSelected(event); setView('edit') } },
      ...
      {showForm && (
        <EventForm
          ...
          onSaved={() => { qc.invalidateQueries({ queryKey: ['events'] }); qc.invalidateQueries({ queryKey: ['next-events'] }); qc.invalidateQueries({ queryKey: ['dashboard-stats'] }); setShowForm(false); setPrefillDate('') }}
        />
      )}
    </div>
  )
...
function SetlistSection({ eventId }: { eventId: string }) {
  ...
  const addSong = useMutation({
    mutationFn: async (songId: string) => {
      const maxPos = setlist.length > 0 ? Math.max(...setlist.map((s: { position: number }) => s.position)) + 1 : 1
  ...
  const addedIds = new Set(setlist.map((s: { song_id: string }) => s.song_id))
  const filteredSongs = allSongs.filter(s => !addedIds.has(s.id) && (!songSearch || s.title.toLowerCase().includes(songSearch.toLowerCase())))
        ...
        {adding && (
          <div className="space-y-2 pb-2 border-b border-[#2a2a2a]">
            ...
            <div className="max-h-40 overflow-y-auto space-y-1">
              ...
              {filteredSongs.length === 0 && <p className="text-xs text-gray-600 px-3 py-2">Keine Songs gefunden</p>}
            </div>
          </div>
        )}
        {setlist.length === 0 ? (
          ...
          (setlist as any[]).map((entry, idx: number) => (
            <div key={entry.id} className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-200 flex-1">{(Array.isArray(entry.songs) ? entry.songs[0]?.title : entry.songs?.title) ?? '—'}</span>
...
function EventForm({ initial, prefillDate, onClose, onSaved, inline }: { initial: Event | null; prefillDate: string; onClose: () => void; onSaved: () => void; inline?: boolean }) {
  const [title, setTitle] = useState(initial?.title ?? '')
  const [type, setType] = useState<string>(initial?.type ?? 'show')
  const [date, setDate] = useState(initial?.date ? toLocalDatetimeValue(initial.date) : prefillDate)
  ...
  if (inline) return <div className="max-w-lg space-y-4"><h2 className="text-lg font-bold text-white">{initial ? 'Event bearbeiten' : 'Neues Event'}</h2>{form}</div>
