...
const GENDERS = ['M', 'W', 'Neutral']
const COLORS = ['Schwarz', 'Weiß']

function itemSubtitle(item: MerchItem) {
  return [item.size, item.gender, item.color].filter(Boolean).join(' · ')
}
...
export default function MerchPage() {
  ...
  const adjustStock = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['low-stock-merch'] })
  ...
  if (view === 'detail' && selected) {
    ...
    const sub = itemSubtitle(fresh)
    return (
      <DetailPanel
        title={fresh.name}
        onBack={() => setView('list')}
        onEdit={() => { setEditing(fresh); setShowForm(true) }}
      >
        <Card>
          <CardBody className="space-y-5">
            ...
            <div className="flex items-center gap-6 flex-wrap">
              ...
              {fresh.price != null && <DetailRow label="Preis">{fresh.price} €</DetailRow>}
            </div>
          </CardBody>
        </Card>
      </DetailPanel>
      ...
      {lowStock.length > 0 && (
        <div className="bg-red-950/50 border border-red-900 rounded-lg px-4 py-3">
          ...
          <div className="flex flex-wrap gap-3">
            {lowStock.map(i => (
              <span key={i.id} className="text-xs text-red-300">
                {i.name}{itemSubtitle(i) ? ` (${itemSubtitle(i)})` : ''} – {i.stock} Stk
              </span>
            ))}
          </div>
        </div>
...
function MerchForm({ initial, onClose, onSaved }: { initial: MerchItem | null; onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      reorder_threshold: parseInt(threshold) || 5,
    ...
    <Modal title={initial ? 'Artikel bearbeiten' : 'Neuer Artikel'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <FormField label="Name">
          <Input value={name} onChange={e => setName(e.target.value)} required placeholder="z.B. Hoodie, T-Shirt..." autoFocus />
        </FormField>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <FormField label="Nachbestellgrenze">
